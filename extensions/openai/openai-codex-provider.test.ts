...
describe("openai codex provider", () => {
  ...
  it("resolves gpt-5.5 with Codex native synthesis metadata when the gpt-5.5 row is absent", () => {
    const provider = buildOpenAICodexProviderPlugin();
  ...
  it("resolves gpt-5.5-pro from a gpt-5.4 runtime template with pro pricing and native limits", () => {
    const provider = buildOpenAICodexProviderPlugin();
  ...
  it("does not catalog-synthesize gpt-5.5 from gpt-5.4 because gpt-5.5 uses direct fallback metadata", () => {
    const provider = buildOpenAICodexProviderPlugin();
    ...
    expect(
      requireArray(entries, "model catalog entries").some((entry) => entry.id === "gpt-5.5"),
    ).toBe(false);
  });

  it("augments gpt-5.5-pro from catalog gpt-5.4 when legacy codex rows are absent", () => {
    const provider = buildOpenAICodexProviderPlugin();
