...
export function createSessionStatusTool(opts?: {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (taskLine && !statusText.includes(taskLine)) {
        statusLines.push(taskLine);
      }
      if (usageLine && !statusText.includes(usageLine)) {
        statusLines.push(usageLine);
      }
      const fullStatusText = statusLines.join("\n");
