...
function getStaticBundledChannelPlugin(id: ChannelId): ChannelPlugin | undefined {
  return id === "webchat" ? webchatPlugin : undefined;
}
...
function listBundledChannelPluginIdsForRoot(
  ...
  return Array.from(
    new Set<ChannelId>([
      ...listBundledChannelMetadata(rootScope).map((metadata) => metadata.manifest.id),
      "webchat",
    ]),
...
function getBundledChannelPluginForRoot(
  ...
  const staticPlugin = getStaticBundledChannelPlugin(id);
  if (staticPlugin) {
    return staticPlugin;
  }
...
function getBundledChannelAccountInspectorForRoot(
  ...
  const staticPlugin = getStaticBundledChannelPlugin(id);
  if (staticPlugin?.config.inspectAccount) {
    return staticPlugin.config.inspectAccount;
  }
...
export function setBundledChannelRuntime(id: ChannelId, runtime: PluginRuntime): void {
  if (getStaticBundledChannelPlugin(id)) {
